...
class DownloadsRepository {
  ...
  BookDetail? getBookSnapshot(String sourceId, String bookId) {
    ...
    try {
      // Hive deserialises nested containers as `Map<dynamic, dynamic>` /
      // `List<dynamic>`. `BookDetail.fromJson` (generated by
      // json_serializable) casts inner fields like `chapters` and
      // `coverHeaders` to `Map<String, dynamic>` directly and throws on
      // the raw Hive types — the resulting TypeError was caught here
      // and silently returned null, leaving the Downloads-screen cover
      // poster blank. Walk the tree once to coerce types before decode.
      final coerced = _deepCoerceMap(raw);
      return BookDetail.fromJson(coerced);
    } catch (e) {
      debugPrint('[downloads] getBookSnapshot decode failed: $e');
      return null;
    }
  }

  /// Recursively rewrites Hive's loose `Map<dynamic, dynamic>` and
  /// `List<dynamic>` into typed forms `json_serializable`-generated
  /// code can consume without casting errors.
  Map<String, dynamic> _deepCoerceMap(Map raw) {
    final out = <String, dynamic>{};
    raw.forEach((k, v) {
      out[k.toString()] = _deepCoerceValue(v);
    });
    return out;
  }

  dynamic _deepCoerceValue(dynamic v) {
    if (v is Map) return _deepCoerceMap(v);
    if (v is List) return v.map(_deepCoerceValue).toList();
    return v;
  }
